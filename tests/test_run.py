...
MOCKED_SESSION_LABEL = "ses-bar"

TESTS_ASSETS = Path(__file__).parent / "data"


# Test 2 use cases:
# - orchestrate_download_bids returns an error: True/None
@pytest.mark.parametrize("orchestrate_download_bids_error", [True, False])
@patch("run.get_analysis_run_level_and_hierarchy")
@patch("run.orchestrate_download_bids")
def test_get_bids_data(
    mock_orchestrate_download_bids,
    mock_get_analysis_run_level_and_hierarchy,
    mocked_context,
    mocked_gear_options,
    orchestrate_download_bids_error,
):
    ...
    mocked_hierarchy_dict = {
        ...
        "session_label": MOCKED_SESSION_LABEL,
    }
    mock_get_analysis_run_level_and_hierarchy.return_value = mocked_hierarchy_dict
    orchestrate_download_bids_return_value = 0
    expected_errors = []
    if orchestrate_download_bids_error:
        orchestrate_download_bids_return_value = 1
        expected_errors = ["BIDS Error(s) detected"]

    mock_orchestrate_download_bids.return_value = orchestrate_download_bids_return_value

    participant_info, errors = run.get_bids_data(
        mocked_context, mocked_gear_options, "my_tree_title"
    )

    assert participant_info["subject_label"] == MOCKED_SUBJECT_LABEL
    # check that the run_label gets sanitized
    assert participant_info["run_label"] == expected_run_label
    assert errors == expected_errors
    assert participant_info["session_label"] == MOCKED_SESSION_LABEL
    mock_get_analysis_run_level_and_hierarchy.assert_called_once()
    mock_orchestrate_download_bids.assert_called_once()
...
def test_main(
    ...
    mocked_parse_config_return = mocked_app_context
...
def test_main_dry_run(
    ...
    mock_save_metadata.assert_called_once_with(
        mocked_context,
        mocked_app_context.analysis_output_dir / "qsiprep",
        {"dry-run": "true"},
    )
    mock_run.assert_not_called()
    mock_post_run.assert_called_once()
